...
针对{{Curr_subj}}学业成绩标准分，我们用逐步回归的方法找到了与之最相关的几个测评任务，建立了如下的预测模型：


```{r present-model-tasks-{{Curr_subj}}}
...
该模型用`r Curr_mod_RPT$n_task`个测评任务成绩预测了{{Curr_subj}}学业成绩`r adj_R2`的变异，即不同学生在{{Curr_subj}}成绩上的差异有`r adj_R2`可以用这几个测评任务成绩代表的能力解释。
...
```{r plot-predict-{{Curr_subj}}, fig.width=8, fig.height=6}
...
```{r predict-namelist-{{Curr_subj}}, echo=FALSE}
